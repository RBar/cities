...
void main() {
  ...
  GetCitiesAndCountriesAtPage usecase;
  setUp(() {
    mockCitiesOfTheWorldRepository = MockCitiesOfTheWorldRepository();
    usecase = GetCitiesAndCountriesAtPage(mockCitiesOfTheWorldRepository);
  });
  const int tpage = 1;
  ...
  test('should return a response from the repository', () async {
    //arrange
    when(mockCitiesOfTheWorldRepository.getCitiesAndCountriesAtPage(any))
        .thenAnswer((_) async => Right(tresponse));
    //act
    final result = await usecase(const CitiesPageParams(page: tpage));
    //assert
    expect(result, Right(tresponse));
    verify(mockCitiesOfTheWorldRepository.getCitiesAndCountriesAtPage(tpage));
