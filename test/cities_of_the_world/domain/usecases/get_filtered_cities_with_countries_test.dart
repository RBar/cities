...
void main() {
  ...
  GetFilteredCitiesWithCountries usecase;
  setUp(() {
    mockCitiesOfTheWorldRepository = MockCitiesOfTheWorldRepository();
    usecase = GetFilteredCitiesWithCountries(mockCitiesOfTheWorldRepository);
  });
  ...
  test('should return a response from the repository', () async {
    //arrange
    when(mockCitiesOfTheWorldRepository.getFilteredCitiesWithCountries(any))
        .thenAnswer((_) async => Right(tresponse));
    //act
    final result = await usecase(const FilteredParams(filter: tfilter));
    //assert
    expect(result, Right(tresponse));
    verify(
        mockCitiesOfTheWorldRepository.getFilteredCitiesWithCountries(tfilter));
